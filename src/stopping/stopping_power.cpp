...
nb::object csda_range(const nb::object& E_MeV_u,
  ...
  // Capture source by value in the lambda
  auto compute = [](const std::vector<std::variant<double, int>>& vec) -> double {
    if (vec.size() < 3) {
      throw std::invalid_argument("Input vector must have at least three elements.");
    }
    double energy = variant_cast<double>(vec[0]);
    ...
    // AT_CSDA_range_g_cm2_single calculates range from E_initial to E_final (0.0)
    double result = AT_CSDA_range_g_cm2_single(energy, 0.0, part_no, mat_no);

    return result;  // Returns g/cm^2
  };
