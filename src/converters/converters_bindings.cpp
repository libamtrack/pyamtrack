...
const char* beta_from_energy_doc = R"pbdoc(
    ...
    Parameters:
        energy_per_nucleon_MeV_u (float | int | numpy.ndarray | list): The energy per nucleon in MeV/u. Can be a single value, a NumPy array, or a Python list.
...
NB_MODULE(converters, m) {
  ...
  m.def("beta_from_energy", &beta_from_energy, nb::arg("energy_per_nucleon_MeV_u"), beta_from_energy_doc);
