...
def test_mixed_parameter_types(electron_energy_MeV):
    ...
    range_many_methods = pyamtrack.stopping.electron_range(electron_energy_MeV, pyamtrack.materials.water_liquid, [1, 2])
    range_many_materials_and_methods = pyamtrack.stopping.electron_range(electron_energy_MeV, [0, 1, pyamtrack.materials.water_liquid], [3, 4, 'tabata'])
    range_numpy_arrays = pyamtrack.stopping.electron_range(electron_energy_MeV, np.array([0, 1, 2],dtype=int), np.array([3, 4, 3],dtype=np.int64))
